...
def egfr(session:Session, patient_cohort:pd.DataFrame, prevalence_point:dt.datetime, mode = "min")->pd.DataFrame:
    """
    ...
    if missing_cols:
        raise MissingColumnError(f"Patient cohort must contain columns: {', '.join(missing_cols)}")
    ...
    egfr_data.loc[egfr_data["serviceidcode"].isin(["QBLAB", "QBLAL"]), "egfr"] = egfr_data.loc[egfr_data["serviceidcode"].isin(["QBLAB", "QBLAL"]), "resultvalue"]

    # We need birthtime and sex from the patient_cohort to calculate eGFR
    egfr_data = egfr_data.merge(
        patient_cohort[["pid", "birthtime", "sex"]], 
        on="pid", 
        how="inner"
    )
    ...
    egfr_data = egfr_data.drop_duplicates(subset=["pid", "observationtime"], keep="first")
    egfr_data = egfr_data.drop(columns=["priority"])

    # Select which egfr result to keep
    if mode == "min":
        egfr_data = egfr_data.sort_values("egfr").drop_duplicates(subset=["pid"], keep="first")
    elif mode == "last":
        egfr_data = egfr_data.sort_values("observationtime").drop_duplicates(subset=["pid"], keep="last")
    elif mode == "all":
        pass
    else:
        raise NotImplementedError(f"Mode '{mode}' is not supported. Must be 'min', 'last', or 'all'")
