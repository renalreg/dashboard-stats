...
def main():
    with get_sessionmaker(SERVER, keypath=KEYPATH)() as session:
        with cached_session(session, min_time_seconds=0.5, expiration=360) as cached:
            ...
            for quarter in range(QUARTER_START - 1, QUARTER_START + NO_OF_QUARTERS - 1):
                ...
                for FACILITY in FACILITIES:
                    ...
                    krt_incident_report = krt_incident(cached, FACILITY, quarter_end, quarter_start)
                    krt_incident_report = age(krt_incident_report, quarter_end)
                    krt_incident_report = adult_paed(krt_incident_report)
                    krt_incident_report = vascular_access(krt_incident_report, cached, quarter_end)
                    ...
                    krt_prevalent_report = krt_prevalent(cached, FACILITY, quarter_end)
                    krt_prevalent_report = age(krt_prevalent_report, quarter_end)
                    krt_prevalent_report = adult_paed(krt_prevalent_report)
                    krt_prevalent_report = vascular_access(krt_prevalent_report, cached, quarter_end)
