...
from ukrdc_stats.calculators.ckd import (
    PrevalentCKDCalculator,
)
...
demographics["birthtime"] = pd.to_datetime(demographics["birthtime"], errors="coerce")
...
# Recode sex 1/2 into Male Female using dictionary
demographics["sex"] = demographics["sex"].replace({"1": "Male", "2": "Female"})
# Recode sex that is not NaN or in ["Male", "Female"] into "Other"
...
demographics = demographics.merge(
    ethnicity_map, how="left", left_on="ethnicgroupcode", right_on="source_code"
)
...
demographics["ethnicgroupcode"] = demographics["ethnicgroupcode"].apply(
    lambda x: x
    if pd.isna(x) or x in ["White", "Asian", "Black", "Unknown"]
    else "Other"
)
...
#    START_DATE = dt.datetime(2024, 10, 1)
    ...
    for x in range(4):
        ...
        # database
        calculator1 = PrevalentCKDCalculator(
            session=session, 
            facility=facility, 
            prevalence_point=END_DATE,
        )
        jfm2 = calculator1.extract_patient_cohort()

        if jfm2 is None:
            print('No CKD patients this quarter')
            continue

        # df has several rows per person as repeat assessments
        jfm2.drop_duplicates(subset=['pid'])

        # Calculate date columns based on END_DATE
        jfm2["year"] = END_DATE.strftime(
            "%Y"
        )  # append Year column with all same value
        jfm2["quarter"] = (
            END_DATE.month + 2
        ) / 3  # append Quarter column with  same value

        # Hardcode some of the metadata
        jfm2["adultpaed"] = "Adult"  # append Adult column with all same value
        jfm2["incidprev"] = "Prevalent"  # append Incidence column with all same value
        jfm2["variable2"] = "Sex"
        jfm2["measure"]   = "Demography"
        jfm2["option"]    = "Number"
        jfm2["healthcarefacilitycode"] = jfm2["sendingfacility"] # only needed till added to the CKDcalculator
        ...
        # Merge patient cohort with the satelite dataframe
        ...
        # Merge patient cohort with the demographics data
        ...
        jfm4.rename(
            columns={"sex_y": "variable"}, inplace=True
        ...
        jfm4 = jfm3.groupby(
            ["ethnicgroupcode_y"] + GROUP_COLUMNS,
            as_index=False,
        ).agg(value=("pid", "count"))  # Count how many rows in each group
        jfm4.rename(
            columns={"ethnicgroupcode_y": "variable"}, inplace=True
...
tableaudf.rename(columns={"admitreasoncode": "dialtplt"}, inplace=True)
tableaudf.head(5)

# Recode gender 1/2 into Male Female using dictionary
tableaudf["dialtplt"] = tableaudf["dialtplt"].replace({"902": "AKC", "903": "NEPH"})
# Recode gender that is not NaN or in ["Male", "Female"] into "Other"
tableaudf["dialtplt"] = tableaudf["dialtplt"].apply(
    lambda x: x if pd.isna(x) or x in ["AKC", "NEPH"] else "OTHER"
)

# Save dataframe to the OUTPUT_DIR location
tableaudf.to_csv(os.path.join(OUTPUT_DIR, "CKDcohort_tableau.csv"), index=True)
