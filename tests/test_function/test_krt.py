...
@patch('ukrdc_stats.calculators.krt.KRTStatsCalculator._query_dialysis_sessions')
def not_test_calculate_median_dialysis_frequency(mock_query_dialysis_sessions, krt_calculator):
    # Mock return value for _query_dialysis_sessions
    mock_query_dialysis_sessions.return_value = pd.DataFrame({
        'pid': [1, 2, 3, 4, 5],
        'fromtime': [
    ...
    # Call the method under test for all subunits
    result_all = krt_calculator._calculate_median_dialysis_frequency(subunit='all')

    # Validate the result for all subunits
    assert result_all.metadata.title == "Median In-Centre Dialysis Frequency"
    assert result_all.metadata.summary == "Histogram of median frequency of dialysis per week."
    assert result_all.data.x == ['<1','1', '2', '3', '>3']
    assert result_all.data.y == [0, 0, 1, 0, 0]

    # Call the method under test for subunit 'A'
    result_subunit_a = krt_calculator._calculate_median_dialysis_frequency(subunit='A')

    # Validate the result for subunit 'A'
    assert result_subunit_a.metadata.title == "Median In-Centre Dialysis Frequency"
    assert result_subunit_a.metadata.summary == "Histogram of median frequency of dialysis per week."
    assert result_subunit_a.data.x == ['<1','1', '2', '3', '>3']
    assert result_subunit_a.data.y == [0, 0, 1, 0, 0]
